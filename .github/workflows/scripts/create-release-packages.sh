...
generate_commands() {
  ...
  for template in agent-commands/*.md; do
    ...
    case $ext in
      toml)
        body=$(printf '%s\n' "$body" | sed 's/\\/\\\\/g')
        { echo "description = \"$description\""; echo; echo "prompt = \"\"\""; echo "$body"; echo "\"\"\""; } > "$output_dir/sunrise.$name.$ext" ;;
      md)
        echo "$body" > "$output_dir/sunrise.$name.$ext" ;;
      agent.md)
        echo "$body" > "$output_dir/sunrise.$name.$ext" ;;
    esac
  done
...
build_variant() {
  ...
  [[ -d agent-commands/templates-for-commands ]] && {
    ...
    echo "Copied agent-commands/templates-for-commands -> .sunrise/templates/templates-for-commands"
  }
...
ls -1 "$GENRELEASES_DIR"/sunrise-template-*-"${NEW_VERSION}".zip
