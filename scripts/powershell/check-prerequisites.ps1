...
if (-not (Test-Path $paths.FEATURE_DIR -PathType Container)) {
    ...
    Write-Output "Run /sunrise.specify first to create the feature structure."
    exit 1
}

if (-not (Test-Path $paths.FEATURE_DESIGN -PathType Leaf)) {
    Write-Output "ERROR: design.md not found in $($paths.FEATURE_DIR)"
    Write-Output "Run /sunrise.design first to create the implementation plan."
    exit 1
}
...
if ($RequireTasks -and -not (Test-Path $paths.TASKS -PathType Leaf)) {
    ...
    Write-Output "Run /sunrise.taskify first to create the task list."
    exit 1
}
