...
# setup-architect.sh - Setup script for /sunrise.architect command
...
ARCH_TEMPLATE=".sunrise/templates/templates-for-commands/arch-template.md"
...
if [ ! -f "docs/ground-rules.md" ]; then
    echo "ERROR: Ground rules file not found: docs/ground-rules.md. Run /sunrise.set-ground-rules first." >&2
    exit 1
fi
...
if [ "$JSON_OUTPUT" = true ]; then
    ...
    echo "  \"GROUND_RULES\": \"docs/ground-rules.md\","
    ...
    echo "   4. Run feature implementation plans with /sunrise.design"
    echo ""
fi
