...
if [[ ! -d "$FEATURE_DIR" ]]; then
    ...
    echo "Run /sunrise.specify first to create the feature structure." >&2
    exit 1
fi

if [[ ! -f "$FEATURE_DESIGN" ]]; then
    echo "ERROR: design.md not found in $FEATURE_DIR" >&2
    echo "Run /sunrise.design first to create the implementation plan." >&2
    exit 1
fi
...
if $REQUIRE_TASKS && [[ ! -f "$TASKS" ]]; then
    ...
    echo "Run /sunrise.taskify first to create the task list." >&2
    exit 1
fi
