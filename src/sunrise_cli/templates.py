...
def copy_local_template(
    ...
    Args:
        is_first_agent: If True, copies shared .sunrise folder. If False, skips it.
    ...
    # Only copy shared .sunrise folder for the first AI assistant
    # This prevents redundant copying when multiple AI assistants are selected
    if is_first_agent:
        # Create .sunrise directory structure
        ...
        # Copy templates to .sunrise/templates with subdirectories
        dest_templates = sunrise_dir / "templates"
        dest_templates.mkdir(exist_ok=True)

        # Copy from agent-commands/templates-for-commands to .sunrise/templates/templates-for-commands
    ...
    # prefixed with "sunrise." and use agent-specific extensions, preventing conflicts.
    ...
    if commands_dir.exists():
        for cmd_file in commands_dir.glob("*.md"):
            ...
            output_file = agent_path / f"sunrise.{cmd_file.stem}{agent_ext}"
...
def download_and_extract_template(
    ...
    *,
    ...
    Args:
        is_first_agent: If True, copies shared .sunrise folder. If False, skips it to avoid redundancy.
    """
    ...
    if local_templates:
        ...
        if template_path:
            ...
            # Default to repo root (assume we're in src/sunrise_cli)
            ...
            if is_current_dir:
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    for item in source_dir.iterdir():
                        # Skip .sunrise folder for non-first agents to avoid redundancy
                        if not is_first_agent and item.name == ".sunrise":
                            continue
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    # Copy items from temp to project_path, skipping .sunrise for non-first agents
                    for item in source_dir.iterdir():
                        # Skip .sunrise folder for non-first agents to avoid redundancy
                        if not is_first_agent and item.name == ".sunrise":
                            continue
