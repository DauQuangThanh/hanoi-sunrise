...
def init(
    ...
    upgrade: bool = typer.Option(False, "--upgrade", help="Upgrade existing Sunrise project by replacing .sunrise and agent folders with latest templates"),
    ...
    """
    Initialize a new Sunrise project from the latest template.
    ...
    Examples:
        ...
        sunrise init --upgrade --ai claude
        sunrise init --here --upgrade

        # Use local templates for development
        sunrise init demo --local-templates --ai claude
        sunrise init demo --local-templates --template-path /path/to/hanoi-sunrise

        # With environment variables
        export RAINBOW_USE_LOCAL_TEMPLATES=1
        export RAINBOW_TEMPLATE_PATH=/path/to/hanoi-sunrise
        sunrise init demo --ai copilot
    """
    ...
    if here:
        ...
        if project_path.exists() and not upgrade:
            # Check if Sunrise is already installed in this directory
            sunrise_dir = project_path / ".sunrise"
            if sunrise_dir.exists():
                # Sunrise is installed, suggest upgrade
                error_panel = Panel(
                    f"Directory '[cyan]{project_name}[/cyan]' already exists with Sunrise installed\n\n"
                    "To upgrade the existing Sunrise installation, use:\n"
                    f"  [cyan]sunrise init {project_name} --upgrade[/cyan]\n\n"
                    "Or choose a different project name.",
                    title="[yellow]Sunrise Already Installed[/yellow]",
                    border_style="yellow",
                    padding=(1, 2)
                )
                console.print()
                ...
                # Directory exists but no Sunrise installation
                error_panel = Panel(
                    f"Directory '[cyan]{project_name}[/cyan]' already exists\n\n"
                    "Sunrise is not installed in this directory. You can:\n"
                    "  • Use [cyan]--force[/cyan] to merge Sunrise files into the existing directory\n"
                    f"  • Use [cyan]sunrise init . --force[/cyan] from within the directory\n"
                ...
                if not force and sys.stdin.isatty():
                    response = typer.confirm("\nDo you want to proceed and merge Sunrise files into this directory?")
                    if response:
    ...
    if upgrade:
        ...
        if not sunrise_dir.exists():
            error_panel = Panel(
                f"No Sunrise project found in '[cyan]{project_path}[/cyan]'\n"
                "The .sunrise folder does not exist. Cannot upgrade.\n\n"
                "Use 'sunrise init' without --upgrade to initialize a new project.",
    ...
    setup_lines = [
        f"[cyan]Sunrise Project {'Upgrade' if is_upgrade_mode else 'Setup'}[/cyan]",
        "",
    ...
    with Live(tracker.render(), console=console, refresh_per_second=8, transient=True) as live:
        ...
        try:
            ...
            if is_upgrade_mode:
                ...
                # Backup .sunrise folder
                sunrise_dir = project_path / ".sunrise"
                if sunrise_dir.exists():
                    backup_sunrise = project_path / f".sunrise.backup.{timestamp}"
                    shutil.copytree(sunrise_dir, backup_sunrise)
                    backup_paths[".sunrise"] = backup_sunrise
            ...
            # Only copy shared .sunrise folder for the first agent to avoid redundancy
    ...
    steps_lines.append("   • [cyan]/sunrise.set-ground-rules[/] - Establish project principles (ground rules)")
    steps_lines.append("   • [cyan]/sunrise.specify[/] - Create feature specification")
    steps_lines.append("   • [cyan]/sunrise.architect[/] [dim](optional, product-level)[/dim] - Define system architecture")
    steps_lines.append("   • [cyan]/sunrise.standardize[/] [dim](optional, product-level)[/dim] - Establish coding standards")
    steps_lines.append("   • [cyan]/sunrise.design[/] - Create technical implementation plan")
    steps_lines.append("   • [cyan]/sunrise.taskify[/] - Generate actionable tasks")
    steps_lines.append("   • [cyan]/sunrise.implement[/] - Execute implementation")
    ...
    enhancement_lines = [
        ...
        "",
        f"○ [cyan]/sunrise.clarify[/] - Ask structured questions before planning (run before [cyan]/sunrise.design[/])",
        f"○ [cyan]/sunrise.analyze[/] - Cross-artifact consistency report (after [cyan]/sunrise.taskify[/], before [cyan]/sunrise.implement[/])",
        f"○ [cyan]/sunrise.checklist[/] - Generate quality checklists (after [cyan]/sunrise.design[/])",
        f"○ [cyan]/sunrise.design-e2e-test[/] - Design end-to-end test specs (product-level)",
        f"○ [cyan]/sunrise.perform-e2e-test[/] - Execute E2E tests and generate reports"
    ]
...
def check():
    ...
    console.print("\n[bold green]Sunrise CLI is ready to use![/bold green]")
...
def version():
    ...
    cli_version = "unknown"
    try:
        cli_version = importlib.metadata.version("sunrise-cli")
    except Exception:
    ...
    panel = Panel(
        info_table,
        title="[bold cyan]Sunrise CLI Information[/bold cyan]",
        border_style="cyan",
        padding=(1, 2)
    )
