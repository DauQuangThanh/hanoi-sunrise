...
handoffs:
  ...
  - label: Get Architecture Review
    ...
    send: true
scripts:
   py: python scripts/python/setup-assess-context.py --json
agent_scripts:
...
/sunrise.assess-context Analyze our e-commerce platform to understand architecture and patterns
# Then follow up with:
# /sunrise.set-ground-rules (create principles based on assessment)
# /sunrise.specify (add new features)

# Assess full stack Node.js/React application
/sunrise.assess-context Analyze our e-commerce platform before adding new payment gateway

# Assess Python Django application
/sunrise.assess-context Review our Django REST API codebase structure and conventions

# Assess with specific focus
/sunrise.assess-context Focus on testing patterns and API integration conventions for our microservices

# Quick assessment for small projects
/sunrise.assess-context Quick assessment of project structure and conventions
```

## Notes

- Run this ONCE per project when first adding Sunrise to existing brownfield codebase
...
- **Recommended workflow**: `/sunrise.assess-context` → `/sunrise.set-ground-rules` → `/sunrise.specify`
- Follow up with `/sunrise.set-ground-rules` to create project principles based on assessment
- Then use `/sunrise.specify` to create feature specifications
