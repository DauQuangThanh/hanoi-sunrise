...
2. **Load context**: Read `docs/ground-rules.md` and all feature specifications from `specs/*/spec.md`. Load ARCH_DOC template (already copied to docs/). Adhere to the principles for maximizing system clarity, structural simplicity, and long-term maintainability.
...
1. **Design deployment architecture**:
   ...
   - Sunrise infrastructure components (compute, storage, networking)
...
2. **Design CI/CD pipeline**:
   ...
   - Sunrise deployment strategy (blue/green, canary, rolling)
...
/sunrise.architect Focus on scalability and multi-region deployment

# Generate architecture with constraints
/sunrise.architect Budget-conscious architecture with AWS services only
```

## Notes

- This command creates **product-level architecture**, not feature-level plans
- Run this AFTER creating feature specs (`/sunrise.specify`) and ground-rules
- Run this BEFORE creating feature implementation plans (`/sunrise.design`)
