...
handoffs: 
  ...
  - label: Execute E2E Tests
    ...
    send: true
scripts:
   py: python scripts/python/setup-design-e2e-test.py --json
agent_scripts:
...
3. **Execute E2E test design workflow**: Follow the structure in E2E_TEST_DOC template to:
   ...
   - Sunrise test environment requirements
...
2. **Define test environments**:
   - Sunrise environment configurations (staging, QA, pre-prod)
...
1. **Create test execution plan**:
   ...
   - Sunrise execution triggers (CI/CD pipeline, manual, scheduled)
...
/sunrise.design-e2e-test

# Generate E2E test plan with specific focus
/sunrise.design-e2e-test Focus on payment and checkout flows with PCI compliance

# Generate E2E test plan with constraints
/sunrise.design-e2e-test Use Cypress for web, Maestro for mobile, prioritize smoke tests
```

## Notes

- This command creates **product-level E2E test plan**, not feature-level tests
- Run this AFTER creating architecture (`/sunrise.architect`)
- Run this BEFORE or ALONGSIDE standards (`/sunrise.standardize`)
