...
**IMPORTANT**: Automatically generate a 'docs:' prefixed git commit message (e.g., 'docs: establish project ground rules v1.0.0' or 'docs: update ground rules to v1.1.0') and commit docs/ground-rules.md and any updated template files upon completion.

1. Load the ground rules template from `.sunrise/templates/templates-for-commands/ground-rules-template.md`.
   - If `docs/ground-rules.md` already exists, read it to extract current values and version information.
...
2. Collect/derive values for placeholders:
   ...
   - If `docs/ground-rules.md` exists, extract current values from it (preserve existing principles unless user requests changes).
   ...
   - If this is the first ground rules document (no existing `docs/ground-rules.md`), start with version 1.0.0.
   - If version bump type ambiguous, propose reasoning before finalizing.

3. Draft the updated ground-rules content (based on the template):
   - Start with the template from `.sunrise/templates/templates-for-commands/ground-rules-template.md`.
...
7. Write the completed ground rules to `docs/ground-rules.md` (overwrite if exists, create if new).
...
The workflow: **Read template from `.sunrise/templates/templates-for-commands/ground-rules-template.md`** → **Copy and fill to `docs/ground-rules.md`** → **Validate and propagate changes to other templates**.

Do not modify the source template at `.sunrise/templates/templates-for-commands/ground-rules-template.md`; always work with the destination file at `docs/ground-rules.md`.
