...
Identify inconsistencies, duplications, ambiguities, and underspecified items across the three core artifacts (`spec.md`, `design.md`, `tasks.md`) before implementation. This command MUST run only after `/sunrise.taskify` has successfully produced a complete `tasks.md`.

## Operating Constraints

**STRICTLY READ-ONLY**: Do **not** modify any files. Output a structured analysis report. Offer an optional remediation plan (user must explicitly approve before any follow-up editing commands would be invoked manually).

**Ground Rules Authority**: The project ground rules (`docs/ground-rules.md`) are **non-negotiable** within this analysis scope. Ground rules conflicts are automatically CRITICAL and require adjustment of the spec, plan, or tasks—not dilution, reinterpretation, or silent ignoring of the principle. If a principle itself needs to change, that must occur in a separate, explicit ground rules update outside `/sunrise.analyze`.
...
- Load `docs/ground-rules.md` for principle validation
...
- If CRITICAL issues exist: Recommend resolving before `/sunrise.implement`
- If only LOW/MEDIUM: User may proceed, but provide improvement suggestions
- Provide explicit command suggestions: e.g., "Run /sunrise.specify with refinement", "Run /sunrise.design to adjust architecture", "Manually edit tasks.md to add coverage for 'performance-metrics'"
