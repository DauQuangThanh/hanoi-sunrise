...
[![Release](https://github.com/dauquangthanh/hanoi-sunrise/actions/workflows/release.yml/badge.svg)](https://github.com/dauquangthanh/hanoi-sunrise/actions/workflows/release.yml)
[![GitHub stars](https://img.shields.io/github/stars/dauquangthanh/hanoi-sunrise?style=social)](https://github.com/dauquangthanh/hanoi-sunrise/stargazers)
[![License](https://img.shields.io/github/license/dauquangthanh/hanoi-sunrise)](https://github.com/dauquangthanh/hanoi-sunrise/blob/main/LICENSE)
[![Documentation](https://img.shields.io/badge/docs-GitHub_Pages-blue)](https://dauquangthanh.github.io/hanoi-sunrise/)
...
- [🔧 Sunrise CLI Reference](#-sunrise-cli-reference)
...
Hanoi Sunrise supports both **Greenfield** (new projects) and **Brownfield** (existing projects) development:
...
**Greenfield** projects start with establishing principles, architecture, and standards before building features. **Brownfield** projects begin with `/sunrise.assess-context` to understand existing architecture and patterns, then follow a streamlined workflow to add new features while maintaining consistency.
...
| **Setup Phase** | `/sunrise.set-ground-rules` | `/sunrise.assess-context` (once per project) |
...
```bash
sunrise init <PROJECT_NAME>
sunrise check
```

<p align="center">
  <img src="./media/hanoi-sunrise.png" alt="Hanoi Sunrise Installation" width="100%"/>
</p>
...
```bash
uv tool install sunrise-cli --force --native-tls --from git+https://github.com/dauquangthanh/hanoi-sunrise.git
```
...
```bash
uvx --native-tls --from git+https://github.com/dauquangthanh/hanoi-sunrise.git sunrise init <PROJECT_NAME>
```
...
Launch your AI assistant in the project. You'll see `/sunrise.*` commands available.
...
```bash
/sunrise.specify Build a photo organizer with albums grouped by date. 
...
```bash
/sunrise.design Use Vite with vanilla HTML, CSS, and JavaScript. 
...
## 🔧 Sunrise CLI Reference

The `sunrise` command supports the following options:

### Commands

| Command     | Description                                                    |
| ------------- | ---------------------------------------------------------------- |
| `init`      | Initialize a new Sunrise project from the latest template      |
...
### `sunrise init` Arguments & Options
...
sunrise init --here --ai copilot

# Force merge into current (non-empty) directory without confirmation
sunrise init . --force --ai copilot
# or
sunrise init --here --force --ai copilot

# Skip git initialization
sunrise init my-project --ai gemini --no-git

# Enable debug output for troubleshooting
sunrise init my-project --ai claude --debug

# Use GitHub token for API requests (helpful for corporate environments)
sunrise init my-project --ai claude --github-token ghp_your_token_here
...
| ------------------ | ------------------------------------------------------------------------------------------------ |
| `SPECIFY_FEATURE` | Override feature detection for non-Git repositories. Set to the feature directory name (e.g., `001-photo-albums`) to work on a specific feature when not using Git branches.<br/>**Must be set in the context of the agent you're working with prior to using `/sunrise.design` or follow-up commands. |
...
Hanoi Sunrise supports three main development scenarios with different workflows:
...
> Having issues with an agent? [Open an issue](https://github.com/dauquangthanh/hanoi-sunrise/issues/new) so we can improve it.
...
- 💬 [Get Support](https://github.com/dauquangthanh/hanoi-sunrise/issues/new) - Ask questions or report issues
- 📄 [View License](./LICENSE) - MIT License
- 🌟 [Star on GitHub](https://github.com/dauquangthanh/hanoi-sunrise) - Support the project
...
You can use the Sunrise CLI to bootstrap your project, which will bring in the required artifacts in your environment. Run:
...
sunrise init <project_name> --ai claude
sunrise init <project_name> --ai gemini
sunrise init <project_name> --ai copilot

# Or in current directory:
sunrise init . --ai claude
sunrise init . --ai codex

# or use --here flag
sunrise init --here --ai claude
sunrise init --here --ai codex

# Force merge into a non-empty current directory
sunrise init . --force --ai claude

# or
sunrise init --here --force --ai claude
```

The CLI will check if you have Claude Code, Gemini CLI, Cursor CLI, Qwen CLI, opencode, Codex CLI, or Amazon Q Developer CLI installed. If you do not, or you prefer to get the templates without checking for the right tools, use `--ignore-agent-tools` with your command:

```bash
sunrise init <project_name> --ai claude --ignore-agent-tools
```
...
You will know that things are configured correctly if you see the `/sunrise.set-ground-rules`, `/sunrise.specify`, `/sunrise.design`, `/sunrise.taskify`, and `/sunrise.implement` commands available.

The first step should be establishing your project's governing principles using the `/sunrise.set-ground-rules` command. This helps ensure consistent decision-making throughout all subsequent development phases:

```text
/sunrise.set-ground-rules Create principles focused on code quality, testing standards, user experience consistency, and performance requirements. Include governance for how these principles should guide technical decisions and implementation choices.
```
...
With your project principles established, you can now create the functional specifications. Use the `/sunrise.specify` command and then provide the concrete requirements for the project you want to develop.
...
1. Use `/sunrise.clarify` (structured) – sequential, coverage-based questioning that records answers in a Clarifications section.
2. Optionally follow up with ad-hoc free-form refinement if something still feels vague.

If you intentionally want to skip clarification (e.g., spike or exploratory prototype), explicitly state that so the agent doesn't block on missing clarifications.

Example free-form refinement prompt (after `/sunrise.clarify` if still needed):
...
#### **STEP 6:** Generate task breakdown with /sunrise.taskify

With the implementation plan validated, you can now break down the plan into specific, actionable tasks that can be executed in the correct order. Use the `/sunrise.taskify` command to automatically generate a detailed task breakdown from your implementation plan:
...
The generated tasks.md provides a clear roadmap for the `/sunrise.implement` command, ensuring systematic implementation that maintains code quality and allows for incremental delivery of user stories.

#### **STEP 7:** Implementation

Once ready, use the `/sunrise.implement` command to execute your implementation plan:

```text
/sunrise.implement
```

The `/sunrise.implement` command will:
...
```text
/sunrise.set-ground-rules Review the context assessment and update project principles to align with the existing codebase patterns. 
...
>[!TIP]
>The `/sunrise.set-ground-rules` command in brownfield projects should reference the context assessment to ensure principles align with existing practices rather than imposing new ones.
...
```text
/sunrise.specify Add a user notification system that sends email alerts when tasks are assigned. 
...
```text
/sunrise.design Follow the existing email service pattern identified in the context assessment. 
...
- 🐛 **Bug Reports:** [Open an issue](https://github.com/dauquangthanh/hanoi-sunrise/issues/new)
- 💡 **Feature Requests:** [Open an issue](https://github.com/dauquangthanh/hanoi-sunrise/issues/new)
- ❓ **Questions:** [Open a discussion](https://github.com/dauquangthanh/hanoi-sunrise/discussions)
